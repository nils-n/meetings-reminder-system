...
def test_can_update_participant_name(
    new_name, expectation, fixture_name, request
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "test-email+3@test.com",
            "test-email+3@test.com",
            "init_participant",
        ),
    ],
)
def test_can_update_participant_email(
    new_email, expectation, fixture_name, request
):
...
def test_can_update_participant_id(
...
def test_wrong_email_raises_type_error(
    new_email, expectation, fixture_name, request
):
...
def test_wrong_name_raises_type_error(
    new_name, expectation, fixture_name, request
):
...
def test_converts_details_correctly_into_table_row(
    ...
    """
    test if details of a participant are converted correctly into a
    table row format
    """
    model = request.getfixturevalue(fixture_name)
